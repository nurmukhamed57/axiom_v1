...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Get the prompt type, fallback to default if not provided or invalid
    const promptType = promptTypes.find(pt => pt.id === selectedPromptType)
      || promptTypes.find(pt => pt.id === DEFAULT_PROMPT_TYPE)!;
